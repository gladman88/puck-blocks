...
| `VehicleCatalog`  | Live vehicle catalog (cars **or** bikes via `vehicleType`) → opens the booking modal. Fetches the public catalog API; deep-link `?vehicle=<id>` opens a card. Optional `showFilters` renders a full type/category/search/date/sort filter bar instead of category tabs (default off — site behavior unchanged); optional `referralCode` / `telegramUser` thread agent attribution and Telegram Mini App prefill into the booking flow (both host-supplied, this block never reads `window.Telegram`/localStorage itself). `CatalogCategory.name_en` is an optional public-catalog-only translation (FMS stays Russian-only, `name` is the source of truth) — every category display goes through the exported `categoryLabel(category, locale)` helper, which falls back to `name` when untranslated. The booking modal's "1-click via Telegram" button POSTs the current selection (dates/accessories/delivery/referral) to the backend's `/catalog/booking-intents/` and redirects with the returned token (`bk_<token>`) — Telegram's 64-char `/start` payload can't carry the selection directly, so the bot resolves the token server-side instead (plans/catalog-telegram-booking-intent/). The delivery/collection picker mirrors the FMS location picker (search a place by name **or** tap a point on a map); the host must expose a Google Maps key (`NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` on the site, `VITE_GOOGLE_MAPS_API_KEY` on the catalog) with **Maps JavaScript API + Places API (New) + Geocoding API** enabled — no key / missing API degrades gracefully to "search unavailable" or a raw-coordinate address |
